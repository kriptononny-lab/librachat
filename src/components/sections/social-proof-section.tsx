...
function AnimatedCounter({ to, suffix = "" }: { to: number; suffix?: string }) {
  ...
  useEffect(() => {
    ...
    const c = animate(count, to, {
      ...
      onUpdate(v) {
        if (ref.current) ref.current.textContent = Math.round(v).toLocaleString("ru") + suffix;
      },
    });
    return c.stop;
...
const STATS = [
  { el: <AnimatedCounter to={1480} suffix="+" />, label: "тестировщиков подтвердили преимущества" },
  ...
  return (
    <section style={{ padding: "120px 0", background: "#100f1d", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
      <div className="container-site">
        ...
        >
          <div className="section-badge" style={{ marginBottom: "16px" }}>НАМ ДОВЕРЯЮТ</div>
          <h2 style={{ fontSize: "clamp(28px, 3vw, 44px)", fontWeight: 800, letterSpacing: "-0.02em", lineHeight: 1.15, color: "#f0eeff" }}>
            Цифры говорят <span style={{ color: "#f0eeff" }}>сами за себя</span>
          </h2>
        </motion.div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "20px" }}>
          {TESTIMONIALS.map((t, i) => (
            <motion.div
              key={t.name}
              initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.42, delay: i * 0.08 }}
            ...
            >
              ...
              <div style={{ padding: "20px", display: "flex", flexDirection: "column", gap: "14px", flex: 1 }}>
                <StarRow />
                <p style={{ fontSize: "14px", lineHeight: 1.75, color: "#a89ec0", flex: 1, fontStyle: "italic" }}>
                  «{t.content}»
                </p>
                <div style={{ padding: "10px 14px", borderRadius: "10px", fontSize: "13px", fontWeight: 600, color: "#9b8ff8", background: "rgba(101,88,224,0.08)", border: "1px solid rgba(101,88,224,0.18)" }}>
                  ↑ {t.result}
                </div>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    <div style={{ width: "40px", height: "40px", borderRadius: "50%", overflow: "hidden", flexShrink: 0, border: "2px solid rgba(101,88,224,0.3)" }}>
                      <Image src={t.photo} alt={t.name} width={40} height={40} style={{ objectFit: "cover", width: "100%", height: "100%" }} />
                    </div>
                    <div>
                      <p style={{ fontSize: "14px", fontWeight: 600, color: "#f0eeff" }}>{t.name}</p>
                      <p style={{ fontSize: "12px", color: "#a89ec0", marginTop: "2px" }}>{t.role}</p>
                    </div>
                  </div>
                  <Link href={t.href} style={{ fontSize: "12px", color: "#6558e0", textDecoration: "none", whiteSpace: "nowrap" }}>
                    Читать кейс →
                  </Link>
                </div>
              </div>
