  ...
  return (
    <Link href={`/learn/${article.slug}`} style={{ display:"flex", flexDirection:"column", borderRadius:"16px", overflow:"hidden", background:"rgba(14,13,25,0.85)", border:"1px solid rgba(255,255,255,0.08)", textDecoration:"none", height:"100%" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:"10px", padding:"16px 18px", flex:1 }}>
        <h3 style={{ fontSize: large ? "15px" : "13px", fontWeight:600, color:"#f2f0ff", lineHeight:1.4 }}>{article.title}</h3>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"12px", fontSize:"12px", color:"#4a4560" }}>
          <div style={{ display:"flex", alignItems:"center", gap:"5px" }}>
            <div style={{ width:"16px", height:"16px", borderRadius:"50%", background:"linear-gradient(135deg,#6558e0,#5248c8)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"8px", fontWeight:700, color:"#fff" }}>
              {article.author[0]}
            </div>
    ...
    <div style={{ display:"flex", minHeight:"100dvh", flexDirection:"column", background:"#07060e" }}>
      ...
      <main style={{ flex:1, paddingTop:"68px" }}>
        ...
        <section style={{ padding:"80px 0 60px", textAlign:"center", position:"relative", overflow:"hidden" }}>
          <div style={{ position:"absolute", top:0, left:"50%", transform:"translateX(-50%) translateY(-33%)", width:"600px", height:"400px", borderRadius:"50%", background:"radial-gradient(circle,rgba(101,88,224,0.12),transparent 70%)", pointerEvents:"none" }} />
          <div className="container-site" style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"24px" }}>
            <div className="section-badge"><span className="badge-dot"/>ОБУЧЕНИЕ И КЕЙСЫ</div>
            <h1 style={{ fontSize:"clamp(32px, 5vw, 60px)", fontWeight:800, letterSpacing:"-0.025em", lineHeight:1.1, color:"#f2f0ff", maxWidth:"700px" }}>
            ...
            <div style={{ display:"flex", gap:"40px" }}>
              {[{ value:`${ARTICLES.length}`, label:"материалов" }, { value:"3", label:"реальных кейса" }, { value:"50K+", label:"читателей" }].map(s => (
            ...
            <div style={{ width:"100%", maxWidth:"560px" }}>
              <div style={{ display:"flex", alignItems:"center", gap:"12px", padding:"14px 18px", borderRadius:"16px", background:"rgba(14,13,25,0.9)", border:"1px solid rgba(255,255,255,0.1)" }}>
                <Search size={18} color="#4a4560" style={{ flexShrink:0 }} />
                <input type="text" placeholder="Найти статью или кейс..." value={search} onChange={e => setSearch(e.target.value)}
                  style={{ flex:1, background:"transparent", border:"none", outline:"none", fontSize:"15px", color:"#f2f0ff" }} />
              </div>
            </div>
              ...
              {FILTERS.map(f => (
                <button key={f.key} onClick={() => setActiveFilter(f.key)}
                  style={{
                    ...
                    background: activeFilter === f.key ? "rgba(101,88,224,0.2)" : "transparent",
                    color: activeFilter === f.key ? "#9b8ff8" : "#a89ec0",
                    outline: activeFilter === f.key ? "1px solid rgba(101,88,224,0.4)" : "1px solid rgba(255,255,255,0.08)",
                  }}>
                  {f.icon} {f.label}
                </button>
              ))}
          ...
          <section style={{ padding:"80px 0", background:"#100f1d", borderTop:"1px solid rgba(255,255,255,0.06)" }}>
            <div className="container-site">
              <div style={{ marginBottom:"40px" }}>
                ...
                <h2 style={{ fontSize:"clamp(26px, 3vw, 40px)", fontWeight:800, color:"#f2f0ff", letterSpacing:"-0.02em" }}>
                    ...
                    <div style={{ padding:"20px", display:"flex", flexDirection:"column", gap:"10px", flex:1 }}>
                      <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
                        <div style={{ width:"36px", height:"36px", borderRadius:"50%", overflow:"hidden", flexShrink:0, border:"2px solid rgba(101,88,224,0.25)" }}>
                          {article.photo ? <Image src={article.photo} alt={article.author} width={36} height={36} style={{ objectFit:"cover", width:"100%", height:"100%" }} /> : <div style={{ width:"100%", height:"100%", background:"#6558e0", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"12px", fontWeight:700, color:"#fff" }}>{article.author[0]}</div>}
                        </div>
                        <div>
                          <p style={{ fontSize:"13px", fontWeight:600, color:"#f2f0ff" }}>{article.author}</p>
                          <p style={{ fontSize:"11px", color:"#a89ec0" }}>{(article as any).authorRole}</p>
                        </div>
                      </div>
                      <h3 style={{ fontSize:"16px", fontWeight:700, color:"#f2f0ff", lineHeight:1.4 }}>{article.title}</h3>
                      <p style={{ fontSize:"13px", color:"#a89ec0", lineHeight:1.6, flex:1 }}>{article.excerpt}</p>
                      <div style={{ fontSize:"13px", color:"#6558e0" }}>Читать кейс →</div>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          <section style={{ padding:"80px 0", background:"#07060e", borderTop:"1px solid rgba(255,255,255,0.06)" }}>
            <div className="container-site">
              <div style={{ marginBottom:"32px" }}>
                ...
                <h2 style={{ fontSize:"clamp(26px, 3vw, 40px)", fontWeight:800, color:"#f2f0ff", letterSpacing:"-0.02em" }}>Избранные материалы</h2>
              </div>
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(280px, 1fr))", gap:"16px" }}>
                {featured.map(article => <Card key={article.slug} article={article} large />)}
              </div>
            </div>
          </section>
        )}
        ...
        <section style={{ padding:"80px 0", background: activeFilter === "все" && !search ? "#100f1d" : "#07060e", borderTop:"1px solid rgba(255,255,255,0.06)" }}>
          <div className="container-site">
            <div style={{ display:"flex", flexWrap:"wrap", alignItems:"center", gap:"12px", marginBottom:"32px" }}>
              <div style={{ marginRight:"auto" }}>
                <h2 style={{ fontSize:"clamp(22px, 2.5vw, 34px)", fontWeight:800, color:"#f2f0ff" }}>
                  {activeFilter === "все" ? "Все материалы" : activeFilter === "статья" ? "Статьи" : activeFilter === "кейс" ? "Кейсы" : "Гайды"}
                </h2>
                <p style={{ fontSize:"13px", color:"#4a4560", marginTop:"4px" }}>
                  Найдено {filteredArticles.length} {filteredArticles.length === 1 ? "материал" : "материалов"}
                </p>
              </div>
            </div>
            ...
            {filteredArticles.length > 0 ? (
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(280px, 1fr))", gap:"16px" }}>
                {filteredArticles.map(article => <Card key={article.slug} article={article} large />)}
              </div>
            ) : (
              <div style={{ textAlign:"center", padding:"60px 20px", color:"#4a4560" }}>
                <BookOpen size={40} style={{ margin:"0 auto 16px", opacity:0.3 }} />
                <p style={{ fontSize:"16px" }}>По запросу ничего не найдено</p>
                <button onClick={() => { setActiveFilter("все"); setSearch(""); }} style={{ marginTop:"12px", fontSize:"13px", color:"#6558e0", background:"none", border:"none", cursor:"pointer" }}>
                  Сбросить фильтры
                </button>
              </div>
            )}
            ...
            {filteredArticles.length > 0 && (
              <div style={{ display:"flex", justifyContent:"center", marginTop:"48px" }}>
                <div style={{ width:"40px", height:"40px", borderRadius:"10px", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"14px", background:"rgba(101,88,224,0.2)", color:"#9b8ff8", border:"1px solid rgba(101,88,224,0.4)" }}>
                  1
                </div>
              </div>
            )}
          ...
          <div className="container-site">
            <div className="cta-inner" style={{ borderRadius:"24px", padding:"72px 40px", textAlign:"center", background:"rgba(14,13,25,0.97)", border:"1px solid rgba(101,88,224,0.18)", position:"relative", overflow:"hidden" }}>
              ...
              <div style={{ position:"relative", display:"flex", flexDirection:"column", alignItems:"center", gap:"20px" }}>
                ...
                <h2 style={{ fontSize:"clamp(26px, 3vw, 44px)", fontWeight:800, letterSpacing:"-0.02em", lineHeight:1.15, color:"#f2f0ff" }}>
                ...
                <div style={{ display:"flex", gap:"12px", flexWrap:"wrap", justifyContent:"center" }}>
                  <Link href="https://librachat.kz/auth" style={{ padding:"14px 32px", borderRadius:"999px", background:"#6558e0", color:"#fff", fontSize:"15px", fontWeight:600, textDecoration:"none", boxShadow:"0 4px 20px rgba(101,88,224,0.4)" }}>Начать бесплатно</Link>
                  <Link href="/pricing" style={{ padding:"14px 32px", borderRadius:"999px", background:"transparent", color:"#f2f0ff", fontSize:"15px", fontWeight:600, textDecoration:"none", border:"1px solid rgba(255,255,255,0.16)" }}>Смотреть тарифы</Link>
                </div>
              </div>
            </div>
          </div>
